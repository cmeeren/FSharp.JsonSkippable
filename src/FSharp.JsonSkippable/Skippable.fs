namespace FSharp.JsonSkippable
...
module Skippable =
  ...
  /// Operators on Skippable.
  module Operators =

    /// Infix operator for Skippable.apply.
    let (<*>) fSkip xSkip = apply fSkip xSkip

    /// Infix operator for Skippable.map.
    let (<!>) f skippable = map f skippable


  open Operators

  /// Lifts a two-parameter function into a function taking Skippable arguments.
  let lift2 f x y =
    f <!> x <*> y

  /// Lifts a three-parameter function into a function taking Skippable arguments.
  let lift3 f x y z =
    f <!> x <*> y <*> z

  /// Lifts a four-parameter function into a function taking Skippable arguments.
  let lift4 f x y z w =
    f <!> x <*> y <*> z <*> w
